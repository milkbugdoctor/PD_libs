...
sub get_amp {
    my ($left, $right, $left1000, $snp, $right1000) = @_;
    my $rc_left = rc($left);
    my $rc_right = rc($right);
    my @tmp = get_positions("$left|$right", $left1000);
    $left_pos = pop(@tmp);
    my @tmp = get_positions("$rc_left|$rc_right", $right1000);
    $right_pos = splice(@tmp, 0, 1);
    substr($right1000, $right_pos) =~ "$rc_left|$rc_right";
    $right_pos += length($&) + length($left1000) + 1;
...
sub start_tm_server {
    $ENV{'NN_PATH'} = '/usr/local/src/MELTING/NNFILES';
    my ($cmd, $params);
    $params = "-Hdnadna -N0.05 -P0.00000005 -T1000 -B";
    $cmd = "/usr/local/bin/melting $params";
