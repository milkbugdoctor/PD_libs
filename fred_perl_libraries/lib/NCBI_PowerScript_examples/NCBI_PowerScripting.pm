...
#version 1.8 1/19/06: added score parsing to elink and elink_by_id
#            12/7/05: altered esearch, epost_set, epost_file to create universal hashes, and added elink_batch_to
#  and elink_by_id_to
#version 1.9 4/26-28/06: altered elink and elink_history to parse the LinkName fields directly; this
# 	changes the output of elink_by_id_to and elink_batch_to; altered esearch_links to allow or
#	arbitrary search terms
#version 1.91 7/13-16/06 - fixed problems in elink_by_id_to and elink_by_id where multiple 
#	destination databases are used
#version 2.0 10/10/06, 11/2/06 - added get_linknames, get_link_report, and extract_links; added efetch_pubchem
#       and get_ftp_file;
#version 2.1 12/28/06, 1/4/07 - modified esummary to support automatic batch downloads (default = 50000) with
#	automatic checking of the number of docsums in each batch
#version 2.2 1/29/07 - added support for http keys in esearch and epost routines for HTTP Get or Post
#version 2.3 4/20/07 - improved handling of no links returned in elink_batch and removed efetch_pubchem (no longer
#		       supported); added comments for Ebot parsing support
#version 2.4 6/12/07 - added support for linkname parameter in elink routines; added routine elink_out
#version 2.5 6/26/07 - modified link batch sizes to reflect improved performance; added HTML output option to elink_out
#	     7/2/07 - added more input sanity checking
#INCLUDE
# ===========================================================================
#
...
@ISA = qw (Exporter);
@EXPORT = qw (read_params egquery esearch esearch_links esummary efetch efetch_batch efetch_pubchem elink
                 epost_uids epost_file elink_history elink_batch elink_batch_to elink_by_id elink_by_id_to
		 elink_out epost_set print_summary print_links print_link_summaries get_uids read_index get_linknames
		 get_link_report extract_links get_ftp_file);
