...
The purpose is to add missing Locus_tag for the ID row in a FASTA file.

License: GNU GPL
Author: Dr. Xiao-Qin Xia

Usage:
	addLocusTag4Fa.py  [options]  parameters

Options:
	-h, --help:	Display this message, then exit
	--version:	Print version information

	-i, --input:	Input file in FASTA format. If not provided, pop the first
		value in {parameters}, or STDIN
	-o, --output:	Output file in FASTA format. If not provided, pop the first
...
def readOneMap(fmap):
	...
	return a dict like {(loc_start, loc_end, strand):Locus_tag, ...}, in which Locus_tag comes from the "Feature ID" column.
	'''
	...
	for line in flocus:
		...
		if start > end: # let start <= end
			start, end = end, start
		rlt[(start, end, strand)] = locus
	return rlt

def readMaps(fmap):
	'''
	fmap: a Tab-delimited file with columns "No" and "FileName"
	return a dict like { No:{(loc_start, loc_end, strand):Locus_tag, ...}, ...}
	'''
...
def Main(fsrc, fobj, fmap, sep='\t', linesep=os.linesep):
	...
	'''
	#r = re.compile(r'>(\d+)(:(\d+)-(\d+):(.*)\s+([+-]))?') # r.match(s).groups(): (No, all_except_No, start, end, Locus_tag, strand)
	r = re.compile(r'>(\d+)(:(\d+)-(\d+):(.*)\s+([+-])(,\s+([+-]))?)?') # r.match(s).groups(): (No, all_except_No, start, end, Locus_tag, strand, (, strand2), strand2)
	fsrc = isinstance(fsrc, str) and open(fsrc) or fsrc
	fobj = isinstance(fobj, str) and open(fobj, 'w') or fobj
	fmap = readMaps(fmap)
	#import Console
	#cs = Console.getconsole()
	#cs.cursor(0)
	#import curses
	#curses.initscr()
	#curses.curs_set(0)
	...
	for line in fsrc:
		...
		#i += 1
		#sys.stdout.write(str(i))
		if line.startswith('>') and fmap: 
			m = r.match(line.strip())
			if not m: sys.stderr.write(line)
			No, all_except_No, start, end, locus_tag, strand, with_strand2, strand2 = vs = m.groups()
			if all_except_No and not locus_tag: # only have No, or has Locus_tag available
				start, end = int(start), int(end)
				if start > end: start, end = end, start
				locus = fmap.get(No, {}).get((start, end, strand), None)
				if locus:
					line = r.sub(r'>\1:\3-\4:%s \6' % locus, line)
				elif strand2 and strand2 != strand:
					locus = fmap.get(No, {}).get((start, end, strand2), None)
					if locus: line = r.sub(r'>\1:\3-\4:%s \6' % locus, line)
		fobj.write(line)
